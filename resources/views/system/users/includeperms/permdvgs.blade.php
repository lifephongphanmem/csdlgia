    ...
    @if($model->level == 'T' || $model->level == 'H' || $model->level == 'DVGS')
        <div class="col-md-3">
            ...
            <table class="table table-striped table-bordered table-hover">
                ...
                <tr>
                    <td>@if($model->level == 'DVGS')<input type="checkbox" {{ (isset($permission->dvgs->create) && $permission->dvgs->create == 1) ? 'checked' : '' }} value="1" name="roles[dvgs][create]"/>@endif</td>
                    <td>Thêm mới</td>
                </tr>
                <tr>
                    <td>@if($model->level == 'DVGS')<input type="checkbox" {{ (isset($permission->dvgs->edit) && $permission->dvgs->edit == 1) ? 'checked' : '' }} value="1" name="roles[dvgs][edit]"/>@endif</td>
                    <td>Chỉnh sửa</td>
                </tr>
                <tr>
                    <td>@if($model->level == 'DVGS')<input type="checkbox" {{ (isset($permission->dvgs->delete) && $permission->dvgs->delete == 1) ? 'checked' : '' }} value="1" name="roles[dvgs][delete]"/>@endif</td>
                    <td>Xóa</td>
                </tr>
                <tr>
                    <td><input type="checkbox" {{ (isset($permission->dvgs->approve) && $permission->dvgs->approve == 1) ? 'checked' : '' }} value="1" name="roles[dvgs][approve]"/></td>
